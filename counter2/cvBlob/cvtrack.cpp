...
#define T(id) tracks.find(IT(id))->second
	...
	void cvUpdateTracks(int upLine,int downLine,int MultiPeople1, int  MultiPeople2,int  MultiPeople3 , long int &inputNum, long int &outputNum,CvBlobs const &blobs, CvTracks &tracks, const double thDistance, const unsigned int thInactive, const unsigned int thActive)
	{
		...
		try
		{
			...
			for (i = 0; i < nBlobs; i++)
			{
				...
				if (c == 0)
				{
					...
					int shouldofwidth = track->maxx - track->minx;
					//output
					if ( track->centroid.y>downLine )
					{
						if(  shouldofwidth <= MultiPeople1 )
						  {
							 outputNum+=1;
						  }
						else if(  shouldofwidth> MultiPeople1 && shouldofwidth<=MultiPeople2 )
						{
							outputNum +=2;
						} 
						else if(  shouldofwidth> MultiPeople2 && shouldofwidth<=MultiPeople3 )
						{
							outputNum +=3;
						}	
						else
					...
					//input
					else if ( track->centroid.y<upLine )
					{
					     if(  shouldofwidth <= MultiPeople1 )
							{
						  		 inputNum+=1;
	...
	void Draw(IplImage *imgSource,vector<CvPoint>Center);
	...
	void Draw(IplImage *imgSource,vector<CvPoint>Center)
	{
		...
		if (size > 1)
		{
			for (int i = 0; i < size - 1; i++)
			{
				...
				{
					CvPoint Temp1 = Center[i];
					CvPoint Temp2 = Center[j];
					cout << "Temp1" << Temp1.x << "\t" << Temp1.y<<endl;
					cout << "Temp1" << Temp2.x << "\t" << Temp2.y<<endl;
					cvLine(imgSource, Temp1, Temp2, CV_RGB(0., 255., 0.), 2);
					cout << "check1" << endl;
				}
			}
		}
		...
		if (mode)
		{
			...
			for (CvTracks::const_iterator it = tracks.begin(); it != tracks.end(); ++it)
			{
				...
				if (!it->second->inactive)
				{
					...
					stringstream buffer;
					buffer << it->first;
					//cout << "CenterY" << (int)it->second->centroid.y << endl;
					//Temp.x = (int)it->second->centroid.x;
					//TempY = (int)it->second->centroid.y;
					//将该坐标存入容器,当有跟踪时，才会
					//TrackY.push_back(TempY);
					//cout << "TempY" << TempY << endl;
					//将不同的轨迹分别存入容器中
					//Track[i].push_back(TrackY);

					//进入时初始帧的中心坐标必然小于等于50
					//TempY是框运动中心纵轴坐标，根据这个坐标判断运动方向
					cvPutText(imgDest, buffer.str().c_str(), cvPoint((int)it->second->centroid.x, (int)it->second->centroid.y), font, CV_RGB(0., 255., 0.));
					//保存每一个轨迹,只判断进入的，初始的TempY小于绿线，但是一个Blob只能加一次
				    //++i;
				}
