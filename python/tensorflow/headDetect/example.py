...
DATASET_PATH = '/home/lxg/codedata/tensorflow/train/' # the dataset file or root folder path.

# Image Parameters
N_CLASSES = 2 # CHANGE HERE, total number of classes
IMG_HEIGHT = 24 # CHANGE HERE, the image height to be resized to
IMG_WIDTH = 24 # CHANGE HERE, the image width to be resized to
...
def read_images(dataset_path, mode, batch_size):
    ...
    # image = tf.random_crop(image, size = [length, IMG_HEIGHT,IMG_WIDTH, CHANNELS])
...
def conv_net(x, n_classes, dropout, reuse, is_training):
    ...
    with tf.name_scope('fc1') as scope:
        ...
        out = tf.nn.relu(matmu, name = scope)
...
with tf.Session() as sess:
    ...
    constant_graph = graph_util.convert_variables_to_constants(sess, sess.graph_def, ["fc1"])
    with tf.gfile.FastGFile('train.pb', mode='wb') as f:
