...
class CDiscountDataset(Dataset):
    ...
    def __init__(self, split, folder, transform=[], mode='train'):
        ...
        label_to_category_id = dict(zip(category_names_df['label'], category_names_df['category_id']))  # just transform
        ...
        if mode=='train':
            print('read img list')
            ids = read_list_from_file(CDISCOUNT_DIR + '/split/' + split, comment='#', func=int) # list int??  split:train_id_v0_5655916
            num_ids = len(ids)
            ...
            df = df[ df['_id'].isin(ids)]  # _id is product id, only save _id in ids
            df = df.reindex(np.repeat(df.index.values, df['num_imgs']), method='ffill')
            df['cum_sum' ] = df.groupby(['_id']).cumcount()  # _id相同的会从0排序
            ...
            test_df = pd.read_csv (CDISCOUNT_DIR + '/test_by_product_id.csv')
            #df.columns
    ...
    def __getitem__(self, index):
        if self.mode=='train':
            ...
            return image, label, index  ## why index??
