...
batch_size = 16  # because the final class fc'units is not sure, so the memory is floated
use_cuda = True
best_loss = float('inf')
train_loss_all = []
...
train_loader = torch.utils.data.DataLoader(train_data,
                        ...
                        shuffle=True,
                        num_workers=4)
test_data = Cdataset_imblance(path = '/home/lxg/codedata/cdiscount/',
                      csv_file = 'test.csv',
                      train = False,
                      transform=transform)
...
# num_epoch = 2; model = inceptionv3(); learning_rate = 0.01; optimizer = torch.optim.SGD(model.parameters(), lr=learning_rate, momentum=0.9)
...
def train(epoch):
    ...
    for batch_idx, (images, labels_level1, labels_subcategory) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        output_level1, output_subcategory = model(images, labels_level1)
        loss = criterion(labels_level1, output_level1, labels_subcategory, output_subcategory)
        loss.backward()
        optimizer.step()
        ...
        if (batch_idx+1) % 10 == 0:
            print ('train Epoch [%d/%d], Iter [%d/%d] lr: %.10f Loss: %.4f accuracy: %.3f subclass: %.3f' \
                   %(epoch+1, num_epoch, batch_idx+1, len(train_loader), learning_rate, 
                        loss.data[0] / len(labels_level1), accuracy_level1, accuracy_subclass))
...
def test(epoch):
    ...
    for batch_idx, (images, labels_level1, labels_subcategory) in enumerate(test_loader):
        ...
        accuracy_subclass = 100. * correct_subclass / total_num
        if (batch_idx+1) % 10 == 0:
            print ('test Epoch [%d/%d], Iter [%d/%d] Loss: %.4f accuracy: %.3f subclass: %.3f' \
                   %(epoch+1, num_epoch, batch_idx+1, len(test_loader), loss.data[0] / len(labels_level1) \
                   , accuracy_level1, accuracy_subclass))
    ...
    print("epoch %d train_loss:%.5f, test_loss %.5f, best_test_loss %.3f, accuracy_level1 %.3f subclass: %.3f\n" % (
            epoch, train_loss, test_loss, best_loss/len(test_loader), accuracy_level1, accuracy_subcategory))
