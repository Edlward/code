...
#endif
{
  ...
  tc = KLTCreateTrackingContext(); // parameter set
  KLTPrintTrackingContext(tc);
  fl = KLTCreateFeatureList(nFeatures);// create memory to store feature

  img1 = pgmReadFile("img0.pgm", NULL, &ncols, &nrows);
  img2 = pgmReadFile("img1.pgm", NULL, &ncols, &nrows);

  KLTSelectGoodFeatures(tc, img1, ncols, nrows, fl); // select good features
  ...
  KLTTrackFeatures(tc, img1, img2, ncols, nrows, fl); // track
